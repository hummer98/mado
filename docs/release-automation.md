...
| **B. Update Homebrew Cask** | [`.github/workflows/update-tap.yml`](../.github/workflows/update-tap.yml) | `release: published` / `workflow_dispatch` | Release published を検知して `hummer98/homebrew-mado` の `Casks/mado.rb` を自動更新 |
...
flowchart TD
    ...
    H --> I[update-tap.yml]
    I --> J{prerelease/draft?}
    J -->|published| K[Casks/mado.rb 更新]
    J -->|skip| L[no-op]
    K --> M[hummer98/homebrew-mado に push]
```
...
| イベント | 挙動 |
|---|---|
| `release: [published]` | `prerelease == false && draft == false` の場合のみ発火 |
...
1. tag/version 解決（`release.tag_name` または `inputs.tag`）
